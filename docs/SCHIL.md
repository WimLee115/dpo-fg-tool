...
> **Lees dit eerst.** Dit stuk is geschreven vóórdat de schil bestond, en het beschrijft de toenmalige toestand op veel plaatsen in de tegenwoordige tijd. Die toestand is ingehaald: `schil/` bevat een volledige Svelte- en Vite-frontend met `package.json` en `vite.config.ts`, `crates/dpofg-schil/tauri.conf.json` bestaat, de raamwerkkeuze is vastgelegd in [`RAAMWERK.md`](RAAMWERK.md), en de bouwstraat kent een eigen baan voor de schil. De redeneringen hieronder zijn niet vervallen — zij zijn uitgevoerd — maar waar staat "er is geen X", lees dan "er was geen X toen dit werd geschreven". Bij het nalopen van 21 augustus 2026 zijn de plaatsen waar dat het meest misleidt hieronder rechtgezet; de stand van de bouw staat in [`../STAND.md`](../STAND.md).
...
**De belangrijkste correctie ten opzichte van het winnende voorstel: de werkbak is geen schermwerk.** Toen dit werd geschreven bestond `AfgeleideVerplichting` uitsluitend voor incidenten, was `verplichtingen_uit_incident` in `crates/dpofg-domain/src/klokken.rs` de enige producent, en was er geen enkele handeling die een verplichting afdeed: `gemeld_op` werd nergens gezet, `dpofg incident` kende geen `melden`, en `AfgeleideVerplichting` had geen voldaan-toestand. *(Alle vier zijn er inmiddels: `verplichtingen_bij_voortdurend_incident` is een tweede producent, `dpofg incident melden` bestaat, en `gemeld_op` wordt gezet in `crates/dpofg-domain/src/incident.rs`. De redenering hieronder is daarmee niet vervallen — zij is uitgevoerd, en dit is hoe zij is uitgevallen.)* Een werkbak bouwen als scherm boven die laag levert een lijst op waaruit niets kan verdwijnen. Daarom staat in dit ontwerp `dpofg werkbak --json` in de kern, vóór er één scherm wordt getekend. Dat heeft drie voordelen: de schil wordt een weergave in plaats van een tweede motor, de aggregatie is te dekken met golden tests per dossiersoort zoals acceptatiecriterium 8 verlangt, en de opdrachtregel krijgt dezelfde lijst — waarmee cron, mail, een afdruk en een agenda-export gratis meekomen op precies het punt waar de schil niets kan, namelijk als hij dicht is.
...
| Veertien losse blokjes, geen doorlopende balk | teller vertaald in een percentage | T1: de brug levert geen deling; `Volledigheidsrapport::percentage()` wordt door de schil niet aangeroepen | de gebruiker kan zelf delen | iemand rekent het alsnog uit | statuspaneel per soort met benoemde tellers |
...
│ Voorbehoud: 19 punten te verifiëren vóór gebruik            [ Tonen ]        │
├──────────────────────────────────────────────────────────────────────────────┤
...
│             art. 12 lid 3 AVG · 1 maand vanaf ontvangst di 21-07             │
...
Daar hangt een tweede gat aan. Er bestond bij het schrijven geen handeling die een verplichting afdeed: `dpofg incident` kende geen `melden`, `gemeld_op` werd nergens gezet, en `ZORG_EINDRAPPORT` is verankerd op precies dat veld. Zonder die handeling kan de bovenste regel van de werkbak niet worden afgevinkt en kan het eindrapportanker nooit ontstaan. Dat is domeinwerk dat vóór het incidentwerkblad af moet zijn.
...
**Ten slotte de telling zelf.** Dit document noemde twaalf kluisloze weergaven, achttien te verifiëren punten in het voorbehoud, achtenzeventig controleregels waarvan er veertien geen evaluatiefunctie hebben, tweeëntwintig opdrachtgroepen en zeven ontvangerrollen. Bij het nalopen van 21 augustus 2026 bleken er drie nog te kloppen en twee niet: het voorbehoud telt negentien punten en er zijn drieëntwintig opdrachtgroepen. Dat is precies het bezwaar dat hier al stond, nu met bewijs: zulke getallen horen niet in een ontwerpdocument thuis te blijven staan maar uit het manifest te komen, zodat een verschoven telling een bouwfout is en geen leesfout. Tot die tijd geldt wat de opdrachten zeggen — `dpofg pakket voorbehoud`, `dpofg controle --dekking` en `dpofg --help` — en niet wat hier staat. Dat is dezelfde discipline die dit ontwerp van zijn gebruikers vraagt.
